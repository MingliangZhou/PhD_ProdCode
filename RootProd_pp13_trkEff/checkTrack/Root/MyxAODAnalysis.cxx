...
float 	 truZvtx;
UInt_t   truNch;
UChar_t  truPt[nTrkMax];
UChar_t  truEta[nTrkMax];
UChar_t  truPhi[nTrkMax];

float 	 recZvtx;
UInt_t   recNch;
UChar_t  recPt[nTrkMax];
UChar_t  recEta[nTrkMax];
UChar_t  recPhi[nTrkMax];
bool		 recPrim[nTrkMax];
bool		 recFake[nTrkMax];
bool		 recQual[nTrkMax];
UChar_t  assPt[nTrkMax];
UChar_t  assEta[nTrkMax];
UShort_t assPhi[nTrkMax];
...
EL::StatusCode MyxAODAnalysis :: initialize ()
{
	...
	tree = new TTree("tree","PYTHIA pp13");
	...
	tree->Branch("truPhi", 	   truPhi,       "truPhi[truNch]/b");
	*/
	...
	tree->Branch("recPhi", 	   recPhi,       "recPhi[recNch]/b");
	...
	tree->Branch("assPhi", 	   assPhi,       "assPhi[recNch]/b");
	*/
...
EL::StatusCode MyxAODAnalysis :: execute ()
{
	...
	for(const auto* ptrRecTrk : *ptrRecTrkCon)
	{
		...
		if(fabs(sin(ptrRecTrk->theta())*(ptrRecTrk->z0()+ptrRecTrk->vz()-recZvtx))<1.0 && fabs(ptrRecTrk->d0())<1.0) recQual[recNch] = true;
		else recQual[recNch] = false;
