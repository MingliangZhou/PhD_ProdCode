...
EL::StatusCode MyxAODAnalysis :: initialize ()
{
	...
	tree = new TTree("tree","2015 Pb+Pb 5.02 TeV");
...
EL::StatusCode MyxAODAnalysis :: execute ()
{
	...
	for(int i=0; i<2; i++)
	{
		if(i==0) sprintf(name,"HLT_mb_sptrk_ion_L1ZDC_A_C_VTE50");
		if(i==1) sprintf(name,"HLT_noalg_mb_L1TE50");
	...
	//------------------------------------------------------
	bool tagGRL = false;
	if(isGoodEvent_PbPb2015(runNo, lbNo, bcid)>0) tagGRL = true;
  ...
  const xAOD::Vertex *ptrVtx = 0;
	...
	*/
		...
		std::cout<<ptrRecTrk->pt()/1000.<<" | "<<trkPt<<" | "<<tmpPt<<std::endl;
	...
	if(nTrk==nTrkMax) return EL::StatusCode::SUCCESS;
	tree->Fill();
