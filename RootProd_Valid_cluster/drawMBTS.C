...
void gen()
{
	...
	TH2D *hCorr = new TH2D("hCorr","",200,-10,10,600,-1,5);
	double sum;
	...
	for(int iEvt=0; iEvt<nEvt; iEvt++)
	{
		...
		for(int i=0; i<nEt; i++)
		{
			if(fabs(etaMin[i]+etaMax[i])/2<3.2) continue;
			if(layer[i]==21 || layer[i]==22 || layer[i]==23) sum += et[i]/1000;
		}
		if(MBTScountA==0 && MBTScountC==0)
		{
			cout<<sum<<endl;
			continue;
		}
		hCorr->Fill(MBTStimeA-MBTStimeC,sum);
	}
	cout<<"Done!"<<endl;
