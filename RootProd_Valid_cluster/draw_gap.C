...
void gen()
{
	...
	TH2D *hGap_A = new TH2D("hGap_A","",50,0,5,50,-0.5,49.5);
	TH2D *hGap_C = new TH2D("hGap_C","",50,0,5,50,-0.5,49.5);
	TH2D *hGap_BA = new TH2D("hGap_BA","",50,0,5,50,-0.5,49.5);
	TH2D *hGap_BC = new TH2D("hGap_BC","",50,0,5,50,-0.5,49.5);
	...
	for(int iEvt=0; iEvt<nEvt; iEvt++)
	{
		...
		if(trigL1[3]>0 && trigL1[4]==0)
		{
			...
			{
				if(2.5+eta[i]<etaGap) etaGap = 2.5+eta[i];
			}
			hGap_A->Fill(etaGap,nTrk,1);
		}
		if(trigL1[3]==0 && trigL1[4]>0)
		{
			hEvt_C->Fill(nTrk);
			...
			hGap_C->Fill(etaGap,nTrk,1);
		}
		if(trigL1[1]>0) hEvt_B->Fill(nTrk);
		if(trigL1[1]>0)
		{
			etaGap = 10;
			for(int i=0; i<nTrk; i++)
		...
		if(trigL1[1]>0)
		{
			...
			hGap_BC->Fill(etaGap,nTrk,1);
		}
	}
	cout<<"Done!"<<endl;

	sprintf(name,"hist_gap.root");
...
void drawComp()
{
	TFile *fIn = new TFile("hist_gap.root");
	...
	styleHist1(hEvt_B);
	TH2D *hGap_A = (TH2D*)fIn->Get("hGap_A");
	styleHist2(hGap_A);
	TH2D *hGap_C = (TH2D*)fIn->Get("hGap_C");
	styleHist2(hGap_C);
	TH2D *hGap_BA = (TH2D*)fIn->Get("hGap_BA");
	styleHist2(hGap_BA);
	TH2D *hGap_BC = (TH2D*)fIn->Get("hGap_BC");
	styleHist2(hGap_BC);

	TH2D *hGapN_A = (TH2D*)hGap_A->Clone("hGapN_A");
	TH2D *hGapN_C = (TH2D*)hGap_C->Clone("hGapN_C");
	TH2D *hGapN_BA = (TH2D*)hGap_BA->Clone("hGapN_BA");
	TH2D *hGapN_BC = (TH2D*)hGap_BC->Clone("hGapN_BC");
	TH2D *hGapN_D = (TH2D*)hGapN_A->Clone("hGapN_D"); // A+C
	TH2D *hGapN_E = (TH2D*)hGapN_A->Clone("hGapN_E"); // A_C + C_A
	for(int i=0; i<hGap_A->GetNbinsX(); i++)
	{
		for(int j=1; j<hGap_A->GetNbinsY(); j++)
		{
			if(hEvt_A->GetBinContent(j+1)>0) hGapN_A->SetBinContent(i+1,j+1,hGapN_A->GetBinContent(i+1,j+1)/hEvt_A->GetBinContent(j+1));
			if(hEvt_C->GetBinContent(j+1)>0) hGapN_C->SetBinContent(i+1,j+1,hGapN_C->GetBinContent(i+1,j+1)/hEvt_C->GetBinContent(j+1));
			if(hEvt_B->GetBinContent(j+1)>0) hGapN_BA->SetBinContent(i+1,j+1,hGapN_BA->GetBinContent(i+1,j+1)/hEvt_B->GetBinContent(j+1));
			if(hEvt_B->GetBinContent(j+1)>0) hGapN_BC->SetBinContent(i+1,j+1,hGapN_BC->GetBinContent(i+1,j+1)/hEvt_B->GetBinContent(j+1));
			hGapN_D->SetBinContent(i+1,j+1,(hGapN_A->GetBinContent(i+1,j+1)+hGapN_C->GetBinContent(i+1,j+1))/2);
			hGapN_E->SetBinContent(i+1,j+1,(hGapN_BA->GetBinContent(i+1,j+1)+hGapN_BC->GetBinContent(i+1,j+1))/2);
		}
	}
	...
	TH1D *hProj_BC[3];
	TH1D *hProj_D[3]; // A+C
	TH1D *hProj_E[3]; // A_C + C_A
	for(int i=0; i<3; i++)
	{
		sprintf(name,"hProj_A_%d",i);
		hProj_A[i] = new TH1D(name,"",50,0,5);
		styleHist1(hProj_A[i]);
		sprintf(name,"hProj_C_%d",i);
		hProj_C[i] = new TH1D(name,"",50,0,5);
		styleHist1(hProj_C[i]);
		sprintf(name,"hProj_BA_%d",i);
		hProj_BA[i] = new TH1D(name,"",50,0,5);
		styleHist1(hProj_BA[i]);
		sprintf(name,"hProj_BC_%d",i);
		hProj_BC[i] = new TH1D(name,"",50,0,5);
		styleHist1(hProj_BC[i]);
		sprintf(name,"hProj_D_%d",i);
		hProj_D[i] = new TH1D(name,"",50,0,5);
		styleHist1(hProj_D[i]);
		sprintf(name,"hProj_E_%d",i);
		hProj_E[i] = new TH1D(name,"",50,0,5);
		styleHist1(hProj_E[i]);
	}
	for(int i=0; i<hGap_A->GetNbinsX(); i++)
	{
		for(int j=0; j<3; j++)
		{
			for(int k=0; k<10; k++)
			{
				hProj_A[j]->AddBinContent(i+1,hGapN_A->GetBinContent(i+1,j*10+k+1)/10);
				hProj_C[j]->AddBinContent(i+1,hGapN_C->GetBinContent(i+1,j*10+k+1)/10);
				hProj_BA[j]->AddBinContent(i+1,hGapN_BA->GetBinContent(i+1,j*10+k+1)/10);
				hProj_BC[j]->AddBinContent(i+1,hGapN_BC->GetBinContent(i+1,j*10+k+1)/10);
				hProj_D[j]->AddBinContent(i+1,hGapN_D->GetBinContent(i+1,j*10+k+1)/10);
				hProj_E[j]->AddBinContent(i+1,hGapN_E->GetBinContent(i+1,j*10+k+1)/10);
			}
		}
	}
	for(int i=0; i<3; i++)
	{
		...
		hProj_E[i]->SetMarkerStyle(mS[0]);
		hProj_E[i]->SetMarkerColor(mC[0]);
		hProj_E[i]->SetLineStyle(lS[0]);
		hProj_E[i]->SetLineColor(lC[0]);
	...
	leg->SetBorderSize(0);
	leg->SetFillColor(0);
	leg->AddEntry(hProj_E[0],"ZDC_A_C","lep");
	...
	{
		if(iC==0) h2Copy = (TH2D*)hGapN_A->Clone("h2Copy");
		if(iC==1) h2Copy = (TH2D*)hGapN_BA->Clone("h2Copy");
		if(iC==2) h2Copy = (TH2D*)hGapN_C->Clone("h2Copy");
		if(iC==3) h2Copy = (TH2D*)hGapN_BC->Clone("h2Copy");
		cOut1->cd(iC+1);
		gPad->SetLogz();
		gPad->SetTicks(1,1);
		...
		if(iC==0) h2Copy->SetTitle("#scale[0.8]{#frac{dN_{evt}}{N_{evt} d#eta_{gap}} | ZDC_A ONLY}");
		if(iC==1) h2Copy->SetTitle("#scale[0.8]{#frac{dN_{evt}}{N_{evt} d#eta_{gap}} | ZDC_A_C}");
		if(iC==2) h2Copy->SetTitle("#scale[0.8]{#frac{dN_{evt}}{N_{evt} d#eta_{gap}} | ZDC_C ONLY}");
		if(iC==3) h2Copy->SetTitle("#scale[0.8]{#frac{dN_{evt}}{N_{evt} d#eta_{gap}} | ZDC_A_C}");
		h2Copy->GetXaxis()->SetTitle("#eta_{gap}");
		h2Copy->GetXaxis()->SetTitleOffset(2.25);
		h2Copy->GetXaxis()->SetRangeUser(0,5);
	...
	for(int iC=0; iC<3; iC++)
	{
		if(iC==0)
		{
			...
			h1CopyM[1] = (TH1D*)hProj_E[0]->Clone("h1CopyM");
		}
		if(iC==1)
		{
			h1CopyM[0] = (TH1D*)hProj_D[1]->Clone("h1CopyM");
			h1CopyM[1] = (TH1D*)hProj_E[1]->Clone("h1CopyM");
		}
		if(iC==2)
		{
			h1CopyM[0] = (TH1D*)hProj_D[2]->Clone("h1CopyM");
			h1CopyM[1] = (TH1D*)hProj_E[2]->Clone("h1CopyM");
		}
		cOut2->cd(iC+1);
		gPad->SetLogy();
		gPad->SetTicks(1,1);
		...
		h1CopyM[0]->GetXaxis()->SetTitle("#eta_{gap}");
		h1CopyM[0]->GetXaxis()->SetTitleOffset(1.25);
		h1CopyM[0]->GetXaxis()->SetRangeUser(0,5);
		h1CopyM[0]->GetYaxis()->SetTitle("dN_{evt} / (d#eta_{gap} N_{evt})");
		h1CopyM[0]->GetYaxis()->SetTitleOffset(1.5);
		h1CopyM[0]->SetMaximum(0.9);
		h1CopyM[0]->SetMinimum(0.0001);
	...
	cOut0->Print("gap_Evt.pdf");
	cOut1->Print("gap_2D.pdf");
	cOut2->Print("gap_1D.pdf");
}
