...
void extractor::Loop()
{
	...
	fChain->SetBranchStatus("trk_blayerPrediction_expectHit",1);
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++){
		...
		for(int reco_track=0;reco_track<trk_n;reco_track++)
		{
			...
			if(trk_pt->at(reco_track)<500) continue;
			if(fabs(trk_eta->at(reco_track))>2.5) continue;
			if(trk_blayerPrediction_expectHit->at(reco_track)==1 && trk_nBLHits->at(reco_track)==0) continue;
			...
			if (trk_nPixHits -> at(reco_track)        > 0  && //loose
					...
					fabs(trk_z0_wrtPV -> at(reco_track)*sin(trk_theta_wrtPV -> at(reco_track)))<1.5)
			{
				Quality += 1;
			}
			if (trk_nPixHits -> at(reco_track)         > 1    && //tight
					...
					fabs(trk_z0_wrtPV -> at(reco_track)*sin(trk_theta_wrtPV -> at(reco_track))) < 1.  &&
			...
			bool chbin=0;
			if (trk_qoverp_wrtPV->at(reco_track)<0) chbin=1;
		...
		for(int iT=0; iT<trk_n; iT++)
		{
			...
			if(Mytrk_Quality[iT]==0) continue;

			if(trk_qoverp_wrtPV->at(iT)>0) trkChg = 1;
			else trkChg = 0;
			if((Mytrk_Quality[iT] & int(pow(2,1)))!=0) trkQual = 1;
			else trkQual = 0;
